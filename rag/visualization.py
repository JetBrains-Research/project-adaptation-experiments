...
def plot_em_scores(path):
    ...
    for ax, category in zip(axes, categories):
        for idx, row in df.iterrows():
            context_lengths = [
                v
                for k, v in row["context_len_mean"].items()
                if row["category"][k] == category
            ]
            em_scores = [
                v["exact_match_valid"]["mean"]
            ...
            # Plot the data
            ax.plot(
                context_lengths, em_scores, marker="o", linestyle="-"
            )  # , label=name)
            ...
            for k, v in row["scores"].items():
                ...
                if row["category"][k] == category:
                    ...
                    if (splitter, scorer) not in scores_dict:
                        ...
                        scores_dict[(splitter, scorer)] = max(
                            scores_dict[(splitter, scorer)], em_score
                        )

        scores_df = pd.DataFrame.from_dict(
            scores_dict, orient="index", columns=["EM Score"]
        ).reset_index()
        scores_df[["Splitter", "Scorer"]] = pd.DataFrame(
            scores_df["index"].tolist(), index=scores_df.index
        )
        scores_df = scores_df.pivot(
            index="Splitter", columns="Scorer", values="EM Score"
        )
