...
class VllmEngineOutput:
    ...
    def get_out_extras(self) -> dict:
        return {
            "engine": "vllm",
            "cumulative_logprob": self.output.outputs[0].cumulative_logprob,
        }
...
def generate(prompts: list[str], llm: LLM, **generation_args) -> list[VllmEngineOutput]:
    outputs = [
        VllmEngineOutput(output=output)
        for output in llm.generate(prompts=prompts, **generation_args)
    ]
    return outputs
...
with jsonlines.open(
    "/home/glukhov/project-adaptation-experiments/data/file_level/prompts.jsonl", "r"
) as reader:
    for data in reader:
...
with jsonlines.open(
    "/home/glukhov/project-adaptation-experiments/data/kl_rag/top_3/prompts.jsonl", "r"
) as reader:
    for data in reader:
        kl_3_prompts.append(data["prompt"])

with jsonlines.open(
    "/home/glukhov/project-adaptation-experiments/data/kl_rag/top_10/prompts.jsonl", "r"
) as reader:
    for data in reader:
        kl_10_prompts.append(data["prompt"])

print(
    len(file_level_prompts),
    len(line_type),
    len(set(line_type)),
    len(gts),
    len(kl_3_preds),
...
for chunk_num in range(len(file_level_prompts)):
    ...
    _fl = file_level_prompts[
        chunk_num * prompts_chunk_size : (chunk_num + 1) * prompts_chunk_size
    ]
    _kl3 = kl_3_prompts[
        chunk_num * prompts_chunk_size : (chunk_num + 1) * prompts_chunk_size
    ]
